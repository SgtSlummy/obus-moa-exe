...
async function backendSupportsCurrentDesktopRuntime(target) {
  return new Promise((resolve) => {
    const request = http.request(
      new URL("/api/runtime/source-freshness", target),
      { method: "GET", timeout: 1_500 },
      (response) => {
        let body = "";
        ...
        response.once("end", () => {
          ...
          try {
            const freshness = JSON.parse(body);
            resolve(
              freshness?.source_current === true &&
                freshness.loaded_source_mtime_ns === freshness.current_source_mtime_ns,
            );
          } catch (_error) {
            resolve(false);
          }
