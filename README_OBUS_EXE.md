...
The AUI is modeled after the useful WarpUI patterns rather than embedding Warp's implementation. `backend/aui.py` owns a versioned, secret-free action and accessibility manifest exposed at `/api/aui/manifest`. It describes surface-bounded actions, keyboard bindings, accessible view roles/value/help, and live-region principles. The SPA loads this contract into the command palette and the visible AUI action rail; action execution remains bound to OBus's existing route, room, receipt, runtime, and workspace functions. The Run workbench also supports compact/comfortable/spacious density, sidebar collapse, responsive viewport modes, route re-input/retry, and bounded workspace file filtering. The feature controllers are progressively extracted under `backend/static/aui/` for events, layout, workspace, runtime, providers, Rooms/Forums, and Memory, with inline compatibility fallbacks. See `docs/obus-aui.md` and `docs/obus-aui-research.md` for the full interaction model and provenance.

The explicit `/api/plan/deliberate` endpoint is review-only and side-effect-free; it returns ephemeral planned room/thread identifiers without creating durable Rooms, Forums, receipts, or provider calls. Execution remains behind the separate automatic deliberation endpoint and configured policy.

The local workspace context panel is explicitly **read-only local context**. Its status and bounded inspection APIs are exposed at `/api/workspace/status`, `/api/workspace/tree`, `/api/workspace/file`, and `/api/workspace/diff`. Configure a root manually; OBus bounds depth, file count, bytes, and text lines, rejects traversal/symlink escapes, omits secret-shaped files and credential directories such as `.aws`, `.ssh`, `.npmrc`, and `.netrc`, and never runs a shell command. A selected bounded text file can be inserted into the next route.
