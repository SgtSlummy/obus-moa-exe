  ...
  const OBusProviders = {
    create({api, state, $, $$, escapeHtml, formatTokens, toast, refresh, openKeyDialog, openKeySetup} = {}) {
      ...
      const bind = (data = state.dashboard) => {
        ...
        $$(".provider-omniroute-sync").forEach((button) => {
          button.onclick = async () => {
            const original = button.textContent; button.disabled = true; button.textContent = "Filling…";
            try {
              const output = await api("/api/providers/omniroute/sync", {method: "POST"});
              await refresh();
              toast(output.message || "OmniRoute providers loaded");
            } catch (error) { toast(error.message, true); }
            finally { button.disabled = false; button.textContent = original; }
          };
        });
