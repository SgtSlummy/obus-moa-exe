  ...
  const OBusRooms = {
    create({api, state, $, $$, escapeHtml, toast, quantumPollInterval, roomCardName} = {}) {
      ...
      const renderRoomDetail = (room, messages, preserveTask = true) => {
        ...
        $("#room-seats").innerHTML = room.card_ids.map((id) => `<span class="badge ${id === room.chymeria.card_id ? "ready" : ""}">${escapeHtml(roomCardName(id))}${id === room.chymeria.card_id ? " · Chymeria" : ""}</span>`).join("");
      ...
      const renderThreads = () => {
        ...
        $$('[data-thread]').forEach((button) => { button.onclick = () => selectThread(button.dataset.thread); });
      };
